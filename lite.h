...
typedef struct RChar RChar;
struct RChar {
	char *text;          /* Define start character */
	char *terminator;    /* Define a typical end pair */
	enum {               /* key, discard or callback */
	 PRS_KEY = 'k',	     /* Use what's in between as a key. */
	 PRS_DISCARD = 'd',  /* Discard what's between. */
	 PRS_CALLBACK = 'c', /* Use a callback. */
	} between;

	/* this is a union */
	char *(*callback)(void *p, char *text);   /* Here is that callback */
	void *data;  /* data that might be used by some function */	
};


typedef struct RMark RMark;
struct RMark {
	RChar *mark;	       /* The grammar marker */
	char between[1024];  /* The text between */
	unsigned int ss, se, es, ee; 
};
...
struct Renderer {
	...
	KVList * kvlist;
	Buffer * sink;
	RChar * list;/*char_t is now RChar (look above and consider changing it)*/
	Buffer * buffer;
	char * name;
	char * str; // or buffer_t *, or FILE *fh
	char * skip;
	char * delims;
	char * special;
};
